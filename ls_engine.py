...
"""
ls_engine.py — Mor Engraved Relief bevel engine
"""
...
def compute_bevel(rgba, size, soften, angle, altitude, depth, hi_opacity, sh_opacity):
    h, w = rgba.shape[:2]
    ...
    return ((hi_rgba * 255).astype(np.uint8), (sh_rgba * 255).astype(np.uint8))
...
def apply_bevel_emboss(drawable, config):
    ...
    hi_arr, sh_arr = compute_bevel(rgba, size, soften, angle, altitude, depth, hi_opacity, sh_opacity)
    log("bevel computed")

    hi_layer = Gimp.Layer.new(image, "mor-engraved-relief-hi", w, h, Gimp.ImageType.RGBA_IMAGE, 100.0, Gimp.LayerMode.SCREEN)
    ...
    sh_layer = Gimp.Layer.new(image, "mor-engraved-relief-sh", w, h, Gimp.ImageType.RGBA_IMAGE, 100.0, Gimp.LayerMode.MULTIPLY)
    ...
    merged.set_name("mor-engraved-relief")
    log("merged into single mor-engraved-relief layer")
